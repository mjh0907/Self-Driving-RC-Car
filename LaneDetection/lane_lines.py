...
def filter_colors(image):
    ...
    image2 = cv2.addWeighted(white_image, 0., yellow_image, 1., 0.)

    return image2

def compute_distances(line_image):
    left_distance=-10000
    right_distance=-10000
    (num_rows, num_cols, colors) = line_image.shape
    non_zero_columns_colors = np.nonzero(line_image[(num_rows-1)])
    if (len(non_zero_columns_colors) > 0):
        print(non_zero_columns_colors[0])
        non_zero_columns = non_zero_columns_colors[0]
        if (len(non_zero_columns) > 0):
            left_edge = non_zero_columns[0]
            right_edge = non_zero_columns[len(non_zero_columns)-1]
            left_distance = num_cols/2-left_edge
            right_distance = right_edge-num_cols/2
...
def annotate_image(image_in):
    ...
    print(compute_distances(line_image))
...
if __name__ == '__main__':
    ...
    annotated_image = annotate_image(mpimg.imread(options.input_file))
    ...
    parser.add_option('-p', '--port', dest="PORT",
                        help='The port which you want the Streaming Viewer to use')

    options, args = parser.parse_args()
    stream_viewer = StreamViewer(options.PORT)
